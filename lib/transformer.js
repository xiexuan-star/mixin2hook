...
export class Transformer {
   ...
   */
  transform() {
    const { deps, data, props, mixins, methods, computed, injects, provides, lifeCycles } = this.collector;
  ...
  lifeCycle(nodeList) {
    this.lifeCycleNode = nodeList.reduce((res, node) => {
      if (t.isObjectMethod(node)) {
        ...
        let methodSource = `()=>${this.getNodeSource(node.body)}`;
        const bodyAst = parser.parse(methodSource);
        const replaceThis = this.replaceThis.bind(this);
        traverse.default(bodyAst, {
          MemberExpression: replaceThis,
